...
def get_body_info(client_id, body):
    return BodyInfo(*p.getBodyInfo(body, physicsClientId=client_id))

def get_base_name(client_id, body):
    return get_body_info(client_id, body).base_name.decode(encoding='UTF-8')

def get_body_name(client_id, body):
    return get_body_info(client_id, body).body_name.decode(encoding='UTF-8')

def get_name(client_id, body):
    name = get_body_name(client_id, body)
    if name == '':
        name = 'body'
...
def has_body(client_id, name):
    try:
        body_from_name(client_id, name)
    except ValueError:
        return False
    return True

def body_from_name(client_id, name):
    for body in get_bodies(client_id):
        if get_body_name(client_id, body) == name:
            return body
    raise ValueError(name)

def remove_body(client_id, body):
    if (client_id, body) in INFO_FROM_BODY:
        del INFO_FROM_BODY[client_id, body]
    return p.removeBody(body, physicsClientId=client_id)

def get_point(client_id, body):
    return get_pose(client_id, body)[0]

def get_quat(client_id, body):
    return get_pose(client_id, body)[1] # [x,y,z,w]

def get_euler(client_id, body):
    return euler_from_quat(get_quat(client_id, body))

def get_base_values(client_id, body):
    return base_values_from_pose(get_pose(client_id, body))

def set_point(client_id, body, point):
    set_pose(client_id, body, (point, get_quat(client_id, body)))

def set_quat(client_id, body, quat):
    set_pose(client_id, body, (get_point(client_id, body), quat))

def set_euler(client_id, body, euler):
    set_quat(client_id, body, quat_from_euler(euler))

def pose_from_pose2d(pose2d):
    x, y, theta = pose2d
    return Pose(Point(x=x, y=y), Euler(yaw=theta))

def set_base_values(client_id, body, values):
    _, _, z = get_point(client_id, body)
    x, y, theta = values
    set_point(client_id, body, (x, y, z))
    set_quat(client_id, body, z_rotation(theta))

def get_velocity(client_id, body):
    linear, angular = p.getBaseVelocity(body, physicsClientId=client_id)
    return linear, angular # [x,y,z], [wx,wy,wz]

def set_velocity(client_id, body, linear=None, angular=None):
    if linear is not None:
        p.resetBaseVelocity(body, linearVelocity=linear, physicsClientId=client_id)
    if angular is not None:
        p.resetBaseVelocity(body, angularVelocity=angular, physicsClientId=client_id)

def is_rigid_body(client_id, body):
    for joint in get_joints(client_id, body):
        if is_movable(client_id, body, joint):
            return False
    return True

def is_fixed_base(client_id, body):
    return get_mass(client_id, body) == STATIC_MASS

def dump_body(client_id, body, fixed=False):
    print('Body id: {} | Name: {} | Rigid: {} | Fixed: {}'.format(
        body, get_body_name(client_id, body), is_rigid_body(client_id, body), is_fixed_base(client_id, body)))
    for joint in get_joints(client_id, body):
        if fixed or is_movable(client_id, body, joint):
            print('Joint id: {} | Name: {} | Type: {} | Circular: {} | Limits: {}'.format(
                joint, get_joint_name(client_id, body, joint), JOINT_TYPES[get_joint_type(client_id, body, joint)],
                is_circular(client_id, body, joint), get_joint_limits(client_id, body, joint)))
    link = NULL_ID
    print('Link id: {} | Name: {} | Mass: {} | Collision: {} | Visual: {}'.format(
        link, get_base_name(client_id, body), get_mass(client_id, body),
        len(get_collision_data(client_id, body, link)), NULL_ID)) # len(get_visual_data(body, link))))
    for link in get_links(client_id, body):
        joint = parent_joint_from_link(link)
        joint_name = JOINT_TYPES[get_joint_type(client_id, body, joint)] if is_fixed(client_id, body, joint) else get_joint_name(client_id, body, joint)
        print('Link id: {} | Name: {} | Joint: {} | Parent: {} | Mass: {} | Collision: {} | Visual: {}'.format(
            link, get_link_name(client_id, body, link), joint_name,
            get_link_name(client_id, body, get_link_parent(client_id, body, link)), get_mass(client_id, body, link),
            len(get_collision_data(client_id, body, link)), NULL_ID)) # len(get_visual_data(body, link))))
...
def dump_world(client_id):
    """print out information for all bodies that're currently in the scene

    """
    for body in get_bodies(client_id):
        dump_body(client_id, body)
        print()

def clone_body(client_id, body, links=None, collision=True, visual=True):
    ...
    client = get_client(client_id) # client is the new client for the body
    if links is None:
        links = get_links(client_id, body)
    #movable_joints = [joint for joint in links if is_movable(body, joint)]
    new_from_original = {}
    base_link = get_link_parent(client_id, body, links[0]) if links else BASE_LINK
    ...
    for i, link in enumerate(links):
        ...
        joint_info = get_joint_info(client_id, body, link)
        dynamics_info = get_dynamics_info(client_id, body, link)
        masses.append(dynamics_info.mass)
        collision_shapes.append(clone_collision_shape(client_id, body, link) if collision else NULL_ID)
        visual_shapes.append(clone_visual_shape(client_id, body, link) if visual else NULL_ID)
        point, quat = get_local_link_pose(client_id, body, link)
    ...
    base_dynamics_info = get_dynamics_info(client_id, body, base_link)
    base_point, base_quat = get_link_pose(client_id, body, base_link)
    new_body = p.createMultiBody(baseMass=base_dynamics_info.mass,
                                 baseCollisionShapeIndex=clone_collision_shape(client_id, body, base_link) if collision else NULL_ID,
                                 baseVisualShapeIndex=clone_visual_shape(client_id, body, base_link) if visual else NULL_ID,
    ...
    for joint, value in zip(range(len(links)), get_joint_positions(client_id, body, links)):
        # TODO: check if movable?
        p.resetJointState(new_body, joint, value, targetVelocity=0, physicsClientId=client_id)
    return new_body

def clone_world(client, exclude=[]):
    ...
    for body in get_bodies(client):
        if body not in exclude:
            new_body = clone_body(client, body, collision=True, visual=visual)
            mapping[body] = new_body
    return mapping

def set_color(client_id, body, color, link=BASE_LINK, shape_index=NULL_ID):
    """
...
def set_texture(client_id, body, texture=None, link=BASE_LINK, shape_index=NULL_ID):
    if texture is None:
        texture = NULL_ID
    return p.changeVisualShape(body, link, shapeIndex=shape_index, textureUniqueId=texture,
                               physicsClientId=client_id)

def vertices_from_link(client_id, body, link):
    ...
    for data in get_collision_data(client_id, body, link):
        vertices.extend(vertices_from_data(data))
    return vertices

def vertices_from_rigid(client_id, body, link=BASE_LINK):
    ...
    assert implies(link == BASE_LINK, get_num_links(client_id, body) == 0)
    try:
        vertices = vertices_from_link(client_id, body, link)
    except RuntimeError:
...
def approximate_as_prism(client_id, body, body_pose=unit_pose(), **kwargs):
    ...
    vertices = apply_affine(body_pose, vertices_from_rigid(client_id, body, **kwargs))
...
def approximate_as_cylinder(client_id, body, **kwargs):
    ...
    Point, tuple of (diameter, height)
        [description]
    """
    center, (width, length, height) = approximate_as_prism(client_id, body, **kwargs)
...
def load_model(client_id, rel_path, pose=None, **kwargs):
    ...
    add_data_path()
    #with LockRenderer():
    body = load_pybullet(client_id, abs_path, **kwargs)
    if pose is not None:
        set_pose(client_id, body, pose)
    return body
